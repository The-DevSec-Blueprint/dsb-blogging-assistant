...
def main(event, _):
    ...
    if action_name == "getVideoTranscript":
        video_id = event["videoId"]

        response = action_get_video_transcript(video_id)
    ...
    if action_name == "generateBlogPost":
        ...
        response = action_generate_blog_post(transcript, video_name, video_type)
...
def action_get_video_transcript(video_id):
    """
    This function takes in a video ID and returns the transcript.
    """
    youtube_client = YouTubeClient()
