...
def main(event, _):
    ...
    if action_name == "getVideoId":
        ...
        return action_get_video_information(video_name, video_url)
    ...
    if action_name == "sendConfirmationEmail":
        ...
        return action_send_video_confirmation_email(
    ...
    if action_name == "generateBlogPost":
        ...
        return action_generate_blog_post(transcript, video_name, video_type)

    if action_name == "commitBlogToGitHub":
        blog_post_contents = event["blogPostContents"]
        video_name = event["videoName"]

        return action_commit_blog_to_github(video_name, blog_post_contents)
    ...
    if action_name == "sendEmail":
        ...
        return action_send_email(commit_id, branch_name, video_name)

    return {"error": "Invalid action name"}


def action_get_video_information(video_name, video_url):
    """
